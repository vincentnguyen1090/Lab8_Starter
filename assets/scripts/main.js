...
function initializeServiceWorker() {
  ...
  // check if servicworker supported in current browser
  if ("servicewWrker" in navigator) {
    // listen for 'load' event on window obj
    window.addEventListener('load', () => {
      // register ./sw.js as a serviceworker
...
async function getRecipes() {
  ...
  // // Parsing recipes JSON obj into an array if found in localStorage
  // const storedRecipes = JSON.parse(localStorage.getItem('recipes'));
  // if (storedRecipes && storedRecipes.length > 0) {
  //   console.log("Recipes loaded from localStorage.");
  //   return storedRecipes; // return early if data found, no need to fetch
  // }
  ...
  // if storedRecipes not found, fetch recipes from network
  return new Promise(async (resolve, reject) => {
    for (const url of RECIPE_URLS) {
        try {
          const response = await fetch(url); // fetch the recipe data from url
          const recipe = await response.json(); // convert response to JSON
          recipeArr.push(recipe); // add recipe to array

          // if we collected all recipes, resolve Promise and save to localStorage
          if(recipeArr.length == RECIPE_URLS.length) {
            saveRecipesToStorage(recipeArr);
            resolve(recipeArr); // recipes = await getRecipes can now be used
          }
        } catch (err) {
          console.log(err);
          reject(err);
